-- Migration: capture caller IP server-side in revoke RPCs when p_ip is omitted

-- We'll prefer explicitly-provided p_ip, then check common request headers (x-forwarded-for, x-real-ip),
-- then finally fall back to inet_client_addr() if available.

CREATE OR REPLACE FUNCTION public.revoke_session_by_token(
  t text,
  p_revoked_by text DEFAULT NULL,
  p_reason text DEFAULT NULL,
  p_ip text DEFAULT NULL,
  p_user_agent text DEFAULT NULL
)
RETURNS int
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  _deleted int := 0;
  _caller_role text := COALESCE(current_setting('jwt.claims.role', true), current_setting('role', true), current_user);
  _headers jsonb := NULL;
  _resolved_ip text := NULL;
BEGIN
  IF _caller_role <> 'service_role' THEN
    RAISE EXCEPTION 'permission denied: revoke_session_by_token requires service_role (caller=%).', _caller_role;
  END IF;

  -- Attempt to resolve client IP server-side when p_ip is not provided
  IF p_ip IS NULL THEN
    BEGIN
      _headers := current_setting('request.headers', true)::jsonb;
    EXCEPTION WHEN OTHERS THEN
      _headers := NULL;
    END;

    -- Check common header locations and extract first IP if a comma-separated list
    _resolved_ip := COALESCE(
      NULLIF(trim(split_part(COALESCE(_headers ->> 'x-forwarded-for', ''), ',', 1)), ''),
      NULLIF(trim(_headers ->> 'x-real-ip'), ''),
      current_setting('request.header.x-real-ip', true),
      (CASE WHEN inet_client_addr() IS NOT NULL THEN inet_client_addr()::text ELSE NULL END)
    );
  ELSE
    _resolved_ip := p_ip;
  END IF;

  -- log candidate sessions with ip & user agent (resolved value may be NULL if unavailable)
  INSERT INTO public.session_revocations(token, user_id, revoked_by, reason, ip_address, user_agent)
  SELECT s.token, s.user_id, COALESCE(p_revoked_by, current_setting('jwt.claims.sub', true), current_user), p_reason, _resolved_ip, p_user_agent
  FROM public.sessions s
  WHERE s.token = t;

  DELETE FROM public.sessions WHERE token = t;
  GET DIAGNOSTICS _deleted = ROW_COUNT;

  RETURN COALESCE(_deleted, 0);
END;
$$;
COMMENT ON FUNCTION public.revoke_session_by_token(text, text, text, text, text) IS 'Delete a session row by token; requires service_role and logs the revocation (server attempts to capture caller IP when p_ip is omitted)';
CREATE OR REPLACE FUNCTION public.revoke_sessions_for_email(
  p_email text,
  p_revoked_by text DEFAULT NULL,
  p_reason text DEFAULT NULL,
  p_ip text DEFAULT NULL,
  p_user_agent text DEFAULT NULL
)
RETURNS int
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  uid uuid;
  deleted_count int := 0;
  _caller_role text := COALESCE(current_setting('jwt.claims.role', true), current_setting('role', true), current_user);
  _headers jsonb := NULL;
  _resolved_ip text := NULL;
BEGIN
  IF _caller_role <> 'service_role' THEN
    RAISE EXCEPTION 'permission denied: revoke_sessions_for_email requires service_role (caller=%).', _caller_role;
  END IF;

  SELECT id INTO uid FROM public.users WHERE lower(email) = lower(p_email) LIMIT 1;
  IF uid IS NULL THEN
    RETURN 0;
  END IF;

  IF p_ip IS NULL THEN
    BEGIN
      _headers := current_setting('request.headers', true)::jsonb;
    EXCEPTION WHEN OTHERS THEN
      _headers := NULL;
    END;

    _resolved_ip := COALESCE(
      NULLIF(trim(split_part(COALESCE(_headers ->> 'x-forwarded-for', ''), ',', 1)), ''),
      NULLIF(trim(_headers ->> 'x-real-ip'), ''),
      current_setting('request.header.x-real-ip', true),
      (CASE WHEN inet_client_addr() IS NOT NULL THEN inet_client_addr()::text ELSE NULL END)
    );
  ELSE
    _resolved_ip := p_ip;
  END IF;

  INSERT INTO public.session_revocations(token, user_id, revoked_by, reason, ip_address, user_agent)
  SELECT s.token, s.user_id, COALESCE(p_revoked_by, current_setting('jwt.claims.sub', true), current_user), p_reason, _resolved_ip, p_user_agent
  FROM public.sessions s
  WHERE s.user_id = uid;

  DELETE FROM public.sessions WHERE user_id = uid;
  GET DIAGNOSTICS deleted_count = ROW_COUNT;

  RETURN COALESCE(deleted_count, 0);
END;
$$;
COMMENT ON FUNCTION public.revoke_sessions_for_email(text, text, text, text, text) IS 'Delete all sessions for a public.users account by email; requires service_role and logs the revocations (server attempts to capture caller IP when p_ip is omitted)';
