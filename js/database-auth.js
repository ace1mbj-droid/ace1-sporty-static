...
class DatabaseAuth {
    ...
    isAdminBypassAllowed(email) {
        if (!email) {
            return false;
        }

        const allowList = ['hello@ace1.in'];
        return allowList.includes(email.trim().toLowerCase());
    }

    async forceAdminSession(email) {
        if (!this.supabase) {
            return { success: false, error: 'Service unavailable. Please try again.' };
        }

        if (!this.isAdminBypassAllowed(email)) {
            return { success: false, error: 'Admin bypass not enabled for this account.' };
        }
        ...
        try {
            ...
            if (data.role !== 'admin') {
                return { success: false, error: 'Bypass restricted to admin roles.' };
            }
            ...
            if (sessionError) {
                console.error('Session creation error (bypass):', sessionError);
            }
            ...
            if (window.securityManager) {
                window.securityManager.logSecurityEvent('ADMIN_BYPASS_LOGIN', {
                    email: email,
                    note: 'Temporary admin login bypass invoked'
                });
            }

            console.warn('⚠️ Admin bypass login executed for', email);
            return { success: true, user };
        } catch (error) {
            console.error('Admin bypass error:', error);
            return { success: false, error: 'Unable to start admin session.' };
        }
    }
