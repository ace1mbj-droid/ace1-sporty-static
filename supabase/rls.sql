...
create policy if not exists insert_orders_user on orders
  for insert with check (auth.uid() = user_id);

-- allow user to update limited fields (e.g., cancel)
create policy if not exists update_own_orders on orders
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- order_items: users can read items of their own orders
create policy if not exists select_order_items_own on order_items
  for select using (
...
create policy if not exists public_select_product_images on product_images
  for select using (true);
