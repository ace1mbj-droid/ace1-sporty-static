...
USING ((select auth.uid()) = user_id OR session_id = current_setting('app.session_id', true));
-- Users can add to their own cart
CREATE POLICY "shopping_carts_insert_self" ON public.shopping_carts FOR INSERT
WITH CHECK ((select auth.uid()) = user_id OR session_id = current_setting('app.session_id', true));
...
USING ((select auth.uid()) = user_id OR session_id = current_setting('app.session_id', true) OR security.is_admin());
